...
def add_reward_trace(plot, plot_data, use_steps=True, avg_epochs=1, name="reward"):
    ...
    if not len(plot_data):
        return
    train_episodes, loss  = zip(*plot_data)
...
def add_rewards(plot, logpath, use_steps=True, avg_constant=20, transform=lambda x: x, name="reward"):
    dirpath = logpath
...
if __name__ == "__main__":
    ...
    for logpath in logpaths:
        add_rewards(rewards_plot, logpath, use_steps=use_steps, avg_constant=avg_constant, transform=transform, name=name)
